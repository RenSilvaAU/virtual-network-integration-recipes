...
resource "azurerm_storage_account" "default" {
  ...
  network_rules {
    ...
    bypass         = ["AzureServices"]
    private_link_access {
      endpoint_resource_id = "/subscriptions/31e50174-228a-451f-9eba-a903458b5fd9/resourcegroups/BGL-NPE-ARG-SwiftSolvePOC-AML2/providers/Microsoft.MachineLearningServices/workspaces/bglnpeuatsydmlwswiftaml2"
...
resource "azurerm_private_endpoint" "st_ple_blob" {
  name                = "BGL-NPE-UAT-SYD-PEP-st-blob2"
  location            = data.azurerm_resource_group.default.location
  resource_group_name = data.azurerm_resource_group.default.name
  subnet_id           = data.azurerm_subnet.ml.id
...
resource "azurerm_private_endpoint" "storage_ple_file" {
  name                = "BGL-NPE-UAT-SYD-PEP-st-file2"
  location            = data.azurerm_resource_group.default.location
  resource_group_name = data.azurerm_resource_group.default.name
  subnet_id           = data.azurerm_subnet.ml.id
...
resource "azurerm_private_endpoint" "storage_ple_table" {
  name                = "BGL-NPE-UAT-SYD-PEP-st-table2"
  location            = data.azurerm_resource_group.default.location
  resource_group_name = data.azurerm_resource_group.default.name
  subnet_id           = data.azurerm_subnet.ml.id
